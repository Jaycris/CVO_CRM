<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 dark:bg-zinc-900 dark:ring-zinc-800">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-zinc-400">
                            @if ($canSeeFullDetails)
                                Sold leads are based on Finance payment records.
                            @else
                                For privacy, sold lead details are hidden. Only monthly sold lead counts are shown.
                            @endif
                        </p>
                    </div>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-zinc-800">
                    <thead class="bg-slate-50 text-left text-xs font-bold uppercase tracking-wide text-slate-500 dark:bg-zinc-900/80 dark:text-zinc-400">
                        ...
                        @else
                            <tr>
                                ...
                                <th class="px-6 py-4">Sold Leads</th>
                                <th class="px-6 py-4">Refunded Sold Leads</th>
                                <th class="px-6 py-4">Disputed Sold Leads</th>
                                <th class="px-6 py-4">Total Sold Leads</th>
                            </tr>
                        @endif
                    </thead>
                    <tbody class="divide-y divide-slate-200 dark:divide-zinc-800">
                        @if ($canSeeFullDetails)
                                ...
                                <tr class="align-top hover:bg-slate-50/70 dark:hover:bg-zinc-800/60">
                                    ...
                                    <td class="px-6 py-4">
                                        <span @class([
                                            ...
                                            {{ $payment->status === 'Payment Success' ? 'Sold' : $payment->status }}
                                        </span>
                                    </td>
                            ...
                            @endforelse
                        @else
                            @forelse ($creditSummaries as $summary)
                                <tr class="align-top hover:bg-slate-50/70 dark:hover:bg-zinc-800/60">
                                    <td class="px-6 py-4 font-semibold text-slate-900 dark:text-zinc-100">{{ $summary['month'] }}</td>
                                    <td class="px-6 py-4 text-emerald-700 dark:text-emerald-300">{{ $summary['sold'] }}</td>
                                    <td class="px-6 py-4 text-rose-700 dark:text-rose-300">{{ $summary['refund'] }}</td>
                                    <td class="px-6 py-4 text-amber-700 dark:text-amber-300">{{ $summary['dispute'] }}</td>
                                    <td class="px-6 py-4 font-semibold text-slate-900 dark:text-zinc-100">{{ $summary['total'] }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-16 text-center text-slate-500 dark:text-zinc-400">
                                        No sold leads yet.
                                    </td>
                                </tr>
                            @endforelse
            ...
            @if ($canSeeFullDetails && $payments->hasPages())
